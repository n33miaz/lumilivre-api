...
> Versão: 2026-05-22 — Baseline V1..V5 + materialized views V3.
...
2. **Pessoas & autenticação** — `student`, `app_user`, `password_reset_token`,
...
```mermaid
erDiagram
    course ||--o{ student : "1:N"
    academic_module ||--o{ student : "1:N"
    study_shift ||--o{ student : "1:N"
    course ||--o{ thesis : "1:N"

    student ||--o| app_user : "0..1:1"
    ...
    student ||--o{ loan_request : "1:N"
    book_copy ||--o{ loan_request : "1:N"

    student ||--o{ reservation : "1:N"
    ...
    student {
      ...
      int course_id FK
      int academic_module_id FK
      int study_shift_id FK
...
- **Soft delete** via `deleted_at TIMESTAMPTZ` em `student`, `book`,
...
- **Email case-insensitive** via extensão `citext` (`student.email`,
  `app_user.email`).
