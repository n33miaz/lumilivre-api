...
`lumilivre-api` e o backend central do ecossistema LumiLivre. Ele concentra regras de negocio, persistencia, autenticacao, integracoes externas, uploads, importacao de planilhas e geracao de relatorios para o painel web administrativo e para o aplicativo mobile dos leitores.
...
- `service`: camada de aplicacao e dominio, onde ficam as regras de emprestimo, solicitacao, livro, leitor, usuario, TCC, importacao e relatorios.
...
- Leitores: cadastro, edicao, exclusao, reset de senha, foto de perfil, enriquecimento de endereco por CEP e ranking.
...
- Emprestimos: criacao, atualizacao, conclusao, exclusao, historico do leitor, dashboard, atrasos e ranking.
...
- Relatorios: geracao de PDF para emprestimos, leitores, livros, exemplares e estatisticas.
- Importacao: carga `.xlsx` de leitores, livros e exemplares com validacao, deduplicacao e processamento em lotes.
- Dados auxiliares: cursos, turnos, modulos, generos, CDDs e enums.

## Autenticacao e Autorizacao

- Login em `/auth/login` aceita `user` como email ou matricula.
- Leitores usam matricula como identificador; admins e bibliotecarios usam email.
...
- `@CanAccessReader` + `ReaderAuthorizationService` bloqueiam acesso cruzado entre leitores; admin/bibliotecario nao sao impactados.
- `AuthRateLimitFilter` (Bucket4j) aplica quota em `/auth/login` e `/auth/esqueci-senha` (HTTP 429 apos N falhas).
- Senha inicial: o login retorna `isInitialPassword=true` quando a senha do leitor e igual a matricula ou a senha administrativa e igual ao email.
...
- Leitor:
  ...
  - Quando leitor tem email, um usuario `ALUNO` e criado com senha inicial igual a matricula.
  - Reset de senha de leitor redefine a senha para a matricula.
...
- Emprestimo:
  ...
  - Leitor com penalidade ativa nao pode receber novo emprestimo.
  - Penalidade expirada e removida durante a tentativa de novo emprestimo.
  - Limite observado: 3 emprestimos ativos por leitor.
  - Exemplar precisa estar `DISPONIVEL`.
  - Ao cadastrar emprestimo, status do emprestimo vira `ATIVO`, exemplar vira `EMPRESTADO` e contador do leitor e incrementado.
  - Emprestimo concluido nao pode ser alterado ou concluido novamente.
  - Ao concluir, exemplar volta para `DISPONIVEL`.
  - Se houver atraso, a penalidade e calculada por dias de atraso e aplicada ao leitor por 7 dias quando for mais grave que a atual.
  - Excluir emprestimo decrementa contador do leitor e libera exemplar quando o emprestimo estava ativo ou atrasado.
...
- Solicitacao:
  ...
  - Leitor com penalidade ativa nao pode solicitar.
...
- Importacao:
  ...
  - Tipos aceitos: `leitor`, `livro`, `exemplar`.
  - Processamento em lotes de 50.
  - Leitores importados geram usuario `ALUNO` com senha inicial igual a matricula.
...
- **Seguranca**: `SecurityConfig` com allowlist explicita + `anyRequest().authenticated()`. Ownership por matricula via `@CanAccessReader` e `ReaderAuthorizationService`. Rate-limit em `/auth/**` com Bucket4j. CORS por perfil via env.
...
- **Novos modulos**: `ReservaService` (fila FIFO com estados), `DashboardService` (metricas agregadas com views materializadas), `RecomendacaoService` (top livros por genero/leitor), `AuditAspect` + `@Auditable` para trilha de auditoria admin.
