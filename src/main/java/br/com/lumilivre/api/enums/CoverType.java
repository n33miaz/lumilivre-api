...
public enum CoverType implements EnumStatus {
    ...
    CoverType(String ptBrCode, String label) {
    ...
    public static CoverType fromPtBrCode(String code) {
        for (CoverType t : values()) {
            if (t.ptBrCode.equalsIgnoreCase(code) || t.name().equalsIgnoreCase(code)) {
                return t;
            }
        }
        throw new IllegalArgumentException("Unknown CoverType: " + code);
    }
}
